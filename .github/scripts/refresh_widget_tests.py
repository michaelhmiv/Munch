...
widgets = Path("src/widgets.test.ts")
text = widgets.read_text()
text = text.replace(
    'expect(source).toContain("Meal logged");',
    'expect(source).toContain("<title>Meal Logged</title>");',
)
widgets.write_text(text)

policy = Path("src/meal-review-policy.test.ts")
text = policy.read_text()
marker = 'test("meal review widget exposes confirm edit and cancel actions"'
if marker not in text:
    raise SystemExit("meal review policy marker not found")
text = text.split(marker, 1)[0] + '''test("meal review widget exposes one primary and one adjustment action", async () => {
...
macros = Path("public/widgets/macros.test.ts")
text = macros.read_text()
marker = "// ---- import widget: the alcohol opt-in"
if marker not in text:
    raise SystemExit("import test marker not found")
replacement = r'''// ---- import widget: the alcohol opt-in ------------------------------------
//
...
'''
macros.write_text(text.split(marker, 1)[0] + replacement)
