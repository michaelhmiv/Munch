...
describe("Cooks canonical type contract", () => {
    test("canonical types and aliases are never silently passed to PostgreSQL", () => {
        for (const type of COOK_EVENT_TYPES) expect(normalizeCookEventType(type)).toBe(type);
        ...
        expect(normalizeCookEventType("temperature measurement")).toBe("temperature_change");
        expect(() => normalizeCookEventType("teleport")).toThrow("Unsupported cook event type");
    });
});
...
describe("September 19 cooking-session inference safeguards", () => {
    ...
    test("a retrospective recap does not create new current-time actions", () => {
        ...
        const events = parseNaturalCookUpdate(summary, submitted, timezone).events;
        expect(events.filter((e) => e.eventType === "rest" || e.eventType === "food_on")).toEqual([]);
    });
    ...
    test("a Friday rub note does not infer a new Saturday preparation", () => {
        ...
        expect(events.some((event) => event.eventType === "food_on")).toBe(false);
