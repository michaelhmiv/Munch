...
import { listOAuthConnections } from "../portal/repository.js";
import {
    deleteMeal,
    deleteWater,
    deleteWeight,
...
function privateJson(c: Parameters<Hono["get"]>[1] extends never ? never : any, data: unknown) {
...
export function createAppRouter(): Hono {
    ...
    app.get("/app", async (c) => c.html(await Bun.file("./public/app.html").text()));
    app.get("/app/*", async (c) => c.html(await Bun.file("./public/app.html").text()));
    ...
    app.get("/api/app/bootstrap", async (c) =>
        privateJson(
            c,
            await getAppBootstrap(c.get("munchUserId"), c.get("munchUserEmail")),
        ),
    );
    ...
    app.get("/api/app/settings", async (c) => {
        ...
        const [bootstrap, subscription, connections] = await Promise.all([
            getAppBootstrap(userId, c.get("munchUserEmail")),
            getSubscriptionSnapshot(userId),
            listOAuthConnections(userId),
        ]);
        return privateJson(c, { ...bootstrap, subscription, connections });
    });
    ...
    app.post("/api/app/water", requireSameOrigin, async (c) => {
        ...
        const result = await insertWater(c.get("munchUserId"), {
            amount_ml: amountMl,
            ...(typeof body.logged_at === "string" ? { logged_at: body.logged_at } : {}),
    ...
    app.post("/api/app/weight", requireSameOrigin, async (c) => {
        ...
        if (!isPlausibleWeightGrams(weightG)) throw new Error("Weight is outside the supported range");
    ...
    app.put("/api/app/preferences", requireSameOrigin, async (c) => {
        ...
        if (typeof body.timezone === "string") {
            validateTz(body.timezone);
            patch.timezone = body.timezone;
        }
        if (body.preferred_weight_unit === null || isWeightUnit(body.preferred_weight_unit)) {
            patch.preferred_weight_unit = body.preferred_weight_unit;
        }
        if (typeof body.widgets_enabled === "boolean") patch.widgets_enabled = body.widgets_enabled;
        if (typeof body.alcohol_tracking_enabled === "boolean") {
            patch.alcohol_tracking_enabled = body.alcohol_tracking_enabled;
        }
        if (body.preferred_drink_unit === null || body.preferred_drink_unit === "us" || body.preferred_drink_unit === "uk") {
    ...
    app.onError((error, c) => {
        ...
        return c.json({ error: "app_request_failed", message }, status, {
            "Cache-Control": "no-store, private",
        });
    });
