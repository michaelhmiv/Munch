...
describe("MCP latency catalog optimization", () => {
    ...
    test(
        "compacts unmapped verbose descriptions to one bounded sentence",
        () => {
            const description = compactToolDescription(
                "example_tool",
            ...
            expect(description).toBe(
                "First sentence explains the tool clearly.",
            );
            expect(description.length).toBeLessThan(261);
        },
    );
    ...
    test(
        "marks superseded compatibility tools private without changing handlers",
        async () => {
            let capturedName = "";
