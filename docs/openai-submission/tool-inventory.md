# OpenAI tool inventory

Generated from the production MCP source. Do not edit manually.

| Tool                          | Source                           | Read only | Open world | Destructive | Idempotent | Output schema |
| ----------------------------- | -------------------------------- | --------: | ---------: | ----------: | ---------: | ------------: |
| `add_grocery_items`           | `src/recipe-planning-tools.ts`   |     false |      false |       false |       true |          true |
| `add_meal_draft_question`     | `src/meal-draft-tools.ts`        |     false |      false |       false |       true |          true |
| `answer_meal_draft_question`  | `src/meal-draft-tools.ts`        |     false |      false |       false |      false |          true |
| `bulk_import_meals`           | `src/mcp.ts`                     |     false |      false |       false |       true |          true |
| `cancel_meal_draft`           | `src/meal-draft-tools.ts`        |     false |      false |        true |      false |          true |
| `compare_cooks`               | `src/cook-tools.ts`              |      true |      false |       false |       true |          true |
| `confirm_meal_draft`          | `src/meal-draft-tools.ts`        |     false |      false |       false |       true |          true |
| `correct_cook_event`          | `src/cook-tools.ts`              |     false |      false |       false |      false |          true |
| `delete_account`              | `src/mcp.ts`                     |     false |      false |        true |      false |          true |
| `delete_cook`                 | `src/cook-tools.ts`              |     false |      false |        true |       true |          true |
| `delete_meal`                 | `src/mcp.ts`                     |     false |      false |        true |       true |          true |
| `delete_recipe`               | `src/recipe-planning-tools.ts`   |     false |      false |        true |       true |          true |
| `delete_saved_food`           | `src/saved-food-tools.ts`        |     false |      false |        true |       true |          true |
| `delete_water`                | `src/mcp.ts`                     |     false |      false |        true |       true |          true |
| `delete_weight`               | `src/mcp.ts`                     |     false |      false |        true |       true |          true |
| `export_meals`                | `src/mcp.ts`                     |     false |      false |       false |      false |          true |
| `find_munch_actions`          | `src/mcp-latency.ts`             |      true |      false |       false |       true |          true |
| `finish_cook`                 | `src/cook-tools.ts`              |     false |      false |       false |       true |          true |
| `get_alcohol_tracking`        | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_connection_status`       | `src/connection-status-tools.ts` |      true |      false |       false |       true |          true |
| `get_cook`                    | `src/cook-tools.ts`              |      true |      false |       false |       true |          true |
| `get_food_details`            | `src/food-tools.ts`              |      true |      false |       false |       true |          true |
| `get_goal_progress`           | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_grocery_list`            | `src/recipe-planning-tools.ts`   |      true |      false |       false |       true |          true |
| `get_meal_details`            | `src/meal-detail-tools.ts`       |      true |      false |       false |       true |          true |
| `get_meal_draft`              | `src/meal-draft-tools.ts`        |      true |      false |       false |       true |          true |
| `get_meal_patterns`           | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_meal_plan`               | `src/recipe-planning-tools.ts`   |      true |      false |       false |       true |          true |
| `get_meals_by_date`           | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_meals_by_date_range`     | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_meals_today`             | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_nutrition_goals`         | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_nutrition_provenance`    | `src/meal-detail-tools.ts`       |      true |      false |       false |       true |          true |
| `get_nutrition_summary`       | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_pantry`                  | `src/inventory/tools.ts`         |      true |      false |       false |       true |          true |
| `get_recipe`                  | `src/recipe-planning-tools.ts`   |      true |      false |       false |       true |          true |
| `get_timezone`                | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_trends`                  | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_water_by_date`           | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_water_today`             | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_weight_by_date`          | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_weight_by_date_range`    | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_weight_today`            | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_weight_trends`           | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_weight_unit`             | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `get_widget_display`          | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `list_saved_foods`            | `src/saved-food-tools.ts`        |      true |      false |       false |       true |          true |
| `log_cook_portion`            | `src/cook-tools.ts`              |     false |      false |       false |       true |          true |
| `log_meal`                    | `src/mcp.ts`                     |     false |      false |       false |      false |          true |
| `log_recipe`                  | `src/recipe-planning-tools.ts`   |     false |      false |       false |       true |          true |
| `log_water`                   | `src/mcp.ts`                     |     false |      false |       false |      false |          true |
| `log_weight`                  | `src/mcp.ts`                     |     false |      false |       false |      false |          true |
| `lookup_barcode`              | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `lookup_food_barcode`         | `src/food-tools.ts`              |      true |      false |       false |       true |          true |
| `mark_grocery_item_purchased` | `src/recipe-planning-tools.ts`   |     false |      false |       false |       true |          true |
| `mark_saved_food_used`        | `src/saved-food-tools.ts`        |     false |      false |       false |      false |          true |
| `parse_recipe_url`            | `src/recipe-import-tools.ts`     |      true |      false |       false |       true |          true |
| `prepare_cook_recipe_draft`   | `src/cook-tools.ts`              |      true |      false |       false |       true |          true |
| `prepare_meal_confirmation`   | `src/meal-draft-tools.ts`        |     false |      false |       false |      false |          true |
| `prepare_meal_review`         | `src/meal-review-tools.ts`       |     false |      false |       false |       true |          true |
| `reconcile_pantry`            | `src/inventory/tools.ts`         |     false |      false |        true |       true |          true |
| `reconcile_purchase`          | `src/inventory/tools.ts`         |     false |      false |       false |       true |          true |
| `record_cook_result`          | `src/cook-tools.ts`              |     false |      false |       false |      false |          true |
| `reopen_cook`                 | `src/cook-tools.ts`              |     false |      false |       false |       true |          true |
| `repeat_cook`                 | `src/cook-tools.ts`              |     false |      false |       false |       true |          true |
| `resolve_meal_review`         | `src/meal-review-tools.ts`       |     false |      false |       false |      false |          true |
| `run_munch_action`            | `src/mcp-latency.ts`             |     false |      false |        true |      false |         false |
| `save_cook_as_recipe`         | `src/cook-tools.ts`              |     false |      false |       false |       true |          true |
| `save_food`                   | `src/saved-food-tools.ts`        |     false |      false |       false |       true |          true |
| `save_meal_as_recipe`         | `src/meal-to-recipe.ts`          |     false |      false |       false |       true |          true |
| `save_recipe`                 | `src/recipe-planning-tools.ts`   |     false |      false |       false |       true |          true |
| `save_recipe_and_plan`        | `src/recipe-planning-tools.ts`   |     false |      false |       false |       true |          true |
| `schedule_recipe`             | `src/recipe-planning-tools.ts`   |     false |      false |       false |       true |          true |
| `search_cooks`                | `src/cook-tools.ts`              |      true |      false |       false |       true |          true |
| `search_foods`                | `src/food-tools.ts`              |      true |      false |       false |       true |          true |
| `search_meals`                | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `search_recipes`              | `src/recipe-planning-tools.ts`   |      true |      false |       false |       true |          true |
| `search_saved_foods`          | `src/saved-food-tools.ts`        |      true |      false |       false |       true |          true |
| `set_alcohol_tracking`        | `src/mcp.ts`                     |     false |      false |       false |       true |          true |
| `set_nutrition_goals`         | `src/mcp.ts`                     |     false |      false |       false |       true |          true |
| `set_preferred_cook`          | `src/cook-tools.ts`              |     false |      false |       false |       true |          true |
| `set_timezone`                | `src/mcp.ts`                     |     false |      false |       false |       true |          true |
| `set_weight_unit`             | `src/mcp.ts`                     |     false |      false |       false |       true |          true |
| `set_widget_display`          | `src/mcp.ts`                     |     false |      false |       false |       true |          true |
| `start_cook`                  | `src/cook-tools.ts`              |     false |      false |       false |       true |          true |
| `start_meal_draft`            | `src/meal-draft-tools.ts`        |     false |      false |       false |      false |          true |
| `start_meal_import`           | `src/mcp.ts`                     |      true |      false |       false |       true |          true |
| `update_cook`                 | `src/cook-tools.ts`              |     false |      false |       false |       true |          true |
| `update_cook_dish`            | `src/cook-tools.ts`              |     false |      false |       false |      false |          true |
| `update_meal`                 | `src/mcp.ts`                     |     false |      false |       false |       true |          true |
| `update_meal_draft`           | `src/meal-draft-tools.ts`        |     false |      false |       false |       true |          true |
| `update_recipe`               | `src/recipe-planning-tools.ts`   |     false |      false |       false |       true |          true |
| `update_water`                | `src/mcp.ts`                     |     false |      false |       false |       true |          true |
| `update_weight`               | `src/mcp.ts`                     |     false |      false |       false |       true |          true |
| `upsert_meal_draft_item`      | `src/meal-draft-tools.ts`        |     false |      false |       false |       true |          true |
