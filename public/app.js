    ...
    const rows = detail.events
        .map(
            (event) =>
                ...
                "</strong>" +
                (event.dish_ids?.length ? "<small>" + escapeHtml(event.dish_ids.map((id) => detail.dishes.find((dish) => dish.id === id)?.name || "Unknown dish").join(" · ")) + "</small>" : "") +
                "<small>" +
...
function openCookEventEditor(eventRecord, cookId, dishes) {
    ...
    const sharedDishes = dishes.map((dish) => '<label><input type="checkbox" name="dish_ids" value="' + escapeHtml(dish.id) + '" ' + (eventRecord.dish_ids?.includes(dish.id) ? "checked" : "") + " />" + escapeHtml(dish.name) + "</label>").join("");
    openDialog(
    ...
    try {
        ...
        if (form.id === "cook-event-edit-form") {
            const eventBody = {
                ...
                dish_ids: values.dish_id ? [values.dish_id] : new FormData(form).getAll("dish_ids").map(String),
