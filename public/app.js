    ...
    const rows = detail.events
        .map(
            (event) =>
                ...
                "</strong><small>" +
                escapeHtml(event.time_precision === "unknown" || !event.event_at ? (event.relative_phrase || "Time unknown") : formatTime(event.event_at, detail.cook.timezone)) +
                " · submitted " +
                escapeHtml(
...
function openCookEventEditor(eventRecord, cookId, dishes) {
    const localTime = eventRecord.event_at
        ? new Date(new Date(eventRecord.event_at).getTime() - new Date(eventRecord.event_at).getTimezoneOffset() * 60000).toISOString().slice(0, 16)
        : "";
    const types = [
        "preparation",
        "season",
    ...
    try {
        ...
        if (form.id === "cook-event-edit-form") {
            const eventBody = {
                ...
                ...(values.event_at && values.time_precision !== "unknown" ? { event_at: new Date(values.event_at).toISOString() } : {}),
