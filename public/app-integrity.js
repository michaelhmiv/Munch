// Compatibility adapter for legacy website renderers.
//
// The backend now returns canonical nutrition goals with the Insights range
// contract, but app.js predates that contract and still passes null targets into
// its generic metric cards. Capture the canonical response and hydrate those
// cards without duplicating any nutrition calculation in the browser. This file
// also retires the old Foods workspace while the saved-food MCP compatibility
// layer remains available server-side.

if (location.pathname === "/app/foods") {
    location.replace("/app/log");
}

document.querySelectorAll('[data-route="foods"]').forEach((node) => node.remove());
...
function syncInsightsGoals() {
    if (location.pathname !== "/app/insights" || !latestInsights?.goals) return;
    ...
    document.querySelectorAll(".summary-card").forEach((card) => {
        ...
        if (detail) detail.textContent = `${formatTarget(target)} ${unit} target`;
