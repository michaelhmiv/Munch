...
export async function handleAccountAction(button, ctx) {
    ...
    if (action === "export-meals") {
        const data = await ctx.api("/api/app/export/meals", {
            method: "POST",
            body: "{}",
            keepPrevious: true,
        });
        if (!data.url) {
            ctx.toast("No meals are available to export yet.", "error");
            return true;
        }
        location.href = data.url;
        return true;
