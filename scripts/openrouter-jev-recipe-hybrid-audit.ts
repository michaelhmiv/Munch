...
const rows: any[] = [];
for (const entry of RECIPE_IMPORT_CORPUS.filter(
    (x) =>
        !process.env.MUNCH_JEV_AUDIT_CASE_ID ||
        x.id === process.env.MUNCH_JEV_AUDIT_CASE_ID,
)) {
    ...
    const foodSearch = {
        search: async (query: string) => ({
            candidates: candidatesFor(query),
            failures: [],
        }),
    };

    const baselineMeter = blankMeter();
    const baselineResolver = new OpenRouterRecipeImportResolver(qwenConfig, {
        fetcher: openRouterMeteredFetcher(baselineMeter),
    });
    try {
        const s = performance.now();
        const draft = await previewRecipeUrl(entry.url, {
            fetchPage: async () => page,
            semanticResolver: baselineResolver,
            foodSearch,
        });
        row.baseline = {
    ...
    const hybridQwen = new OpenRouterRecipeImportResolver(qwenConfig, {
        fetcher: openRouterMeteredFetcher(hybridQwenMeter),
    });
    ...
    const hybridResolver = new HybridRecipeImportResolver(
        hybridQwen,
        decisionClient,
    );
    try {
        ...
        const draft = await previewRecipeUrl(entry.url, {
            fetchPage: async () => page,
            semanticResolver: hybridResolver,
            foodSearch,
        });
        row.hybrid = {
            durationMs: performance.now() - s,
            qwenMeter: hybridQwenMeter,
            jevMeter,
            quality: draftQuality(draft),
        };
    } catch (e) {
    ...
    rows.push(row);
    writeFileSync(
        "artifacts/openrouter-jev-recipe-hybrid-partial.json",
    ...
    console.log(
        "[hybrid_recipe] " +
            JSON.stringify({
                id: entry.id,
                baselineMs: Math.round(row.baseline?.durationMs || 0),
                hybridMs: Math.round(row.hybrid?.durationMs || 0),
                baselineOk: row.baseline?.quality?.ok ?? false,
                hybridOk: row.hybrid?.quality?.ok ?? false,
                baselineCalls: row.baseline?.meter?.calls ?? 0,
                hybridQwenCalls: row.hybrid?.qwenMeter?.calls ?? 0,
                hybridJevCalls: row.hybrid?.jevMeter?.calls ?? 0,
            }),
    );
    await Bun.sleep(250);
}
...
function sum(path: (r: any) => number) {
    return comparable.reduce((s, r) => s + path(r), 0);
}
function percentile(values: number[], p: number) {
    const a = [...values].sort((x, y) => x - y);
    return a[Math.max(0, Math.ceil(a.length * p) - 1)] ?? 0;
}
const bTimes = comparable.map((r) => r.baseline.durationMs),
    hTimes = comparable.map((r) => r.hybrid.durationMs);
const baselineCost = sum((r) => r.baseline.meter.costUsd);
const hybridCost = sum(
    (r) => r.hybrid.qwenMeter.costUsd + r.hybrid.jevMeter.costUsd,
);
const summary = {
    generatedAt: new Date().toISOString(),
    requested: RECIPE_IMPORT_CORPUS.length,
    fetched: rows.filter((r) => !r.fetchError).length,
    comparable: comparable.length,
    baselinePassed: comparable.filter((r) => r.baseline.quality.ok).length,
    hybridPassed: comparable.filter((r) => r.hybrid.quality.ok).length,
    baselineDecoySelections: sum((r) => r.baseline.quality.decoySelections),
    hybridDecoySelections: sum((r) => r.hybrid.quality.decoySelections),
    baseline: {
        p50Ms: percentile(bTimes, 0.5),
        p95Ms: percentile(bTimes, 0.95),
        meanMs: sum((r) => r.baseline.durationMs) / (comparable.length || 1),
        qwenCalls: sum((r) => r.baseline.meter.calls),
        qwenRetries: sum((r) => r.baseline.meter.retries),
        costUsd: baselineCost,
    },
    hybrid: {
        p50Ms: percentile(hTimes, 0.5),
        p95Ms: percentile(hTimes, 0.95),
        meanMs: sum((r) => r.hybrid.durationMs) / (comparable.length || 1),
        qwenCalls: sum((r) => r.hybrid.qwenMeter.calls),
        qwenRetries: sum((r) => r.hybrid.qwenMeter.retries),
        jevCalls: sum((r) => r.hybrid.jevMeter.calls),
        jevRetries: sum((r) => r.hybrid.jevMeter.retries),
        costUsd: hybridCost,
        qwenCostUsd: sum((r) => r.hybrid.qwenMeter.costUsd),
        jevCostUsd: sum((r) => r.hybrid.jevMeter.costUsd),
    },
    speedupP50: percentile(bTimes, 0.5) / percentile(hTimes, 0.5),
    speedupMean:
        sum((r) => r.baseline.durationMs) /
        (comparable.length || 1) /
        (sum((r) => r.hybrid.durationMs) / (comparable.length || 1)),
    costRatio: hybridCost / (baselineCost || 1),
};
writeFileSync(
    "artifacts/openrouter-jev-recipe-hybrid-audit.json",
...
const md = [
    ...
    "",
    "Fetched/comparable corpus entries: **" +
        summary.comparable +
        "/" +
        summary.requested +
        "**.",
    ...
    "| Current Qwen | " +
        ...
        summary.comparable +
        " | " +
        Math.round(summary.baseline.p50Ms) +
        " ms | " +
        Math.round(summary.baseline.p95Ms) +
        " ms | " +
        summary.baseline.qwenCalls +
        " | 0 | " +
        usd(summary.baseline.costUsd) +
        " |",
    "| Hybrid | " +
        summary.hybridPassed +
        "/" +
        summary.comparable +
        " | " +
        Math.round(summary.hybrid.p50Ms) +
        " ms | " +
        Math.round(summary.hybrid.p95Ms) +
        " ms | " +
        summary.hybrid.qwenCalls +
        " | " +
        summary.hybrid.jevCalls +
        " | " +
        usd(summary.hybrid.costUsd) +
        " |",
    "",
    "Median end-to-end AI-stage speedup: **" +
