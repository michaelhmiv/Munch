...
# MCP Pantry: keep the three-tool catalog, add opt-in planning detail and
# best-effort enrichment after explicit acquisitions/reconciliation.
replace(
...
# Website Pantry: planning reads, enrichment after writes, and an outcome-oriented
# non-mutating meal-ideas endpoint.
replace(
...
# Existing recipe search gains optional Pantry-aware ranking instead of creating
# another MCP tool.
replace(
...
replace(
    ...
    '''    search_recipes: {
        ...
        pantry_candidates: z.array(pantryRecipeCandidateOutputSchema).optional(),
    },''',
)
replace(
    "src/recipe-planning-tools.ts",
    ...
    '''                    const recipes = await searchRecipes({
                    ...
                    if (pantry_match) {
                        ...
                        if (query) {
                            ...
                            pantryCandidates = pantryCandidates.filter((candidate) =>
...
print("Pantry intelligence surface wiring applied.")
