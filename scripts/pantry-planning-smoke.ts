...
if (process.env.MUNCH_PANTRY_PLANNING_ENABLED !== "true") {
    throw new Error("Pantry planning smoke requires MUNCH_PANTRY_PLANNING_ENABLED=true");
}
...
const pantrySeed = await reconcilePantry({
    ...
    operations: [
        { action: "acquire", name: "Ground beef", quantity: 1, unit: "lb", location: "fridge" },
        { action: "acquire", name: "Rice", quantity: 3, unit: "cup", location: "pantry" },
        { action: "acquire", name: "Soy sauce", quantityMode: "presence_only", location: "pantry" },
        { action: "acquire", name: "Garlic", quantityMode: "presence_only", location: "pantry" },
        { action: "acquire", name: "Ginger", quantityMode: "presence_only", location: "fridge" },
        { action: "acquire", name: "Smoked paprika", quantityMode: "presence_only", location: "pantry" },
        { action: "acquire", name: "Cottage cheese", quantity: 24, unit: "oz", location: "fridge" },
        { action: "acquire", name: "Spinach", quantityMode: "presence_only", location: "fridge" },
    ],
});
...
const profileSeed: Array<{
    ...
    { key: "ground beef", category: "protein", roles: ["main", "protein"], protein: 26, calories: 200 },
    { key: "rice", category: "grain_starch", roles: ["base", "starch"], protein: 2.7, calories: 130 },
    { key: "soy sauce", category: "sauce_condiment", roles: ["east-asian", "flavor-builder", "sauce"] },
    { key: "garlic", category: "aromatic", roles: ["aromatic", "flavor-builder"] },
    { key: "ginger", category: "aromatic", roles: ["aromatic", "east-asian", "flavor-builder"] },
    { key: "smoked paprika", category: "spice", roles: ["flavor-builder", "seasoning", "smoky"] },
    { key: "cottage cheese", category: "dairy", roles: ["creamy", "dairy", "protein", "topping"], protein: 12, calories: 100 },
    { key: "spinach", category: "produce", roles: ["produce", "side", "vegetable"], protein: 3, calories: 23 },
];

await withUserDatabase(owner.userId, async (tx) => {
    for (const profile of profileSeed) {
        const item = byName.get(profile.key);
        if (!item) throw new Error(`Missing seeded Pantry item: ${profile.key}`);
        await tx`
...
await saveRecipe({
    ...
    recipe: {
        ...
        cookingMinutes: 20,
        instructions: ["Brown beef.", "Build the soy, garlic, and ginger sauce.", "Serve over rice."],
        ingredients: [
...
await saveRecipe({
    ...
    recipe: {
        ...
        ingredients: [ingredient("Ground beef", 45, 380), ingredient("Rice", 5, 200)],
    },
});
await saveRecipe({
    userId: owner.userId,
...
if (ranked[coherentIndex]?.flavor_support.coverage !== 1) {
    throw new Error("Pantry recipe ranking did not recognize complete flavor support");
}
if (ranked[missingIndex]?.availability.readiness !== "missing_core") {
    throw new Error("Pantry recipe ranking did not identify a missing core protein");
}
if ((ranked[missingIndex]?.score ?? 0) >= (ranked[coherentIndex]?.score ?? 0)) {
    throw new Error("Missing-core recipe was not penalized below ready coherent meal");
}

const profileIds = pantrySeed.operations.map((operation) => operation.item.id);
const viewerProfiles = await getStoredPlanningProfiles(viewer.userId, profileIds);
if (viewerProfiles.size !== profileIds.length) {
    throw new Error("Household viewer could not read shared Pantry planning profiles");
}
const outsiderProfiles = await getStoredPlanningProfiles(outsider.userId, profileIds);
...
if (!viewerWriteDenied) {
    throw new Error("Household viewer unexpectedly wrote a shared Pantry profile");
}
