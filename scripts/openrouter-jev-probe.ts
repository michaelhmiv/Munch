...
const body = {
  ...
  questions: {
    best_candidate: {
      type: "choice",
      instructions: "Choose the single food database candidate that best matches the user's full context for nutrition logging. Choose NO_MATCH when every candidate materially conflicts.",
      criteria: {
...
if (answer?.type !== "choice") throw new Error("Jev response contained no choice answer");
if (answer.choice !== "c1") throw new Error(`Expected c1, got ${answer.choice}`);
if (typeof answer.confidence !== "number") throw new Error("Missing confidence");
