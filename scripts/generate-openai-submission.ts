...
function createSubmission(entries: ToolInventoryEntry[]) {
    ...
    return {
        ...
        test_cases: [
            {
                description:
                    "Prepare a meal review before permanently logging a described meal.",
                user_prompt:
                ...
                expected_output:
                    "Returns a reviewable breakfast draft with item quantities, nutrition estimates, sources, assumptions, and any question needed before confirmation.",
                expected_output_url: null,
            },
            {
                description:
                    "Permanently confirm a previously reviewed meal draft.",
                user_prompt:
                    "Everything in that breakfast review is correct. Confirm and log it.",
                file_attachment_urls: null,
                tools_triggered: "confirm_meal_draft",
                expected_output:
                    "Logs the confirmed meal once and returns the persisted meal summary without duplicating an already confirmed draft.",
                expected_output_url: null,
            },
            {
                description:
                    "Retrieve the authenticated user's nutrition summary for a bounded date range.",
                user_prompt:
                    "Show my calorie and macro totals for the last seven days.",
                file_attachment_urls: null,
                tools_triggered: "get_nutrition_summary",
                expected_output:
                    "Returns the user's seven-day totals and daily breakdown with enough context to identify missing or estimated data.",
                expected_output_url: null,
            },
            {
                description:
                    "Look up a packaged food by barcode using configured food-data providers.",
                user_prompt:
                ...
                expected_output:
                    "Returns the matching packaged-food record, serving basis, nutrients, provider attribution, and a clear not-found result when unavailable.",
                expected_output_url: null,
            },
            {
                description:
                    "Save a recipe, schedule it, and add requested ingredients to a grocery list atomically.",
                user_prompt:
                ...
                expected_output:
                    "Returns the saved recipe revision, scheduled meal, and grocery changes while keeping personal or household ownership explicit.",
                expected_output_url: null,
            },
        ],
        negative_test_cases: [
