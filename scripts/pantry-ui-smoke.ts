...
for (const endpoint of [
    ...
    if (!routes.includes(endpoint) || !js.includes(endpoint)) {
        throw new Error(`Pantry web contract is not wired end-to-end: ${endpoint}`);
    }
}

if (!routes.includes("requireWebSession") || !routes.includes("requireSameOrigin")) {
    throw new Error("Pantry web writes are missing session/CSRF enforcement");
}
if (!routes.includes('capabilities.tier !== "premium"')) {
    throw new Error("Pantry web surface is missing premium entitlement enforcement");
}
...
if (!index.includes("10 * 1024 * 1024")) {
    throw new Error("Global request limit does not admit bounded Pantry image uploads");
}
if (!css.includes("@media(max-width:760px)")) {
    throw new Error("Pantry workspace is missing its mobile responsive treatment");
}
if (/localStorage|sessionStorage/.test(js)) {
    throw new Error("Pantry client must not persist inventory or receipt data in browser storage");
}
if (/receipt_image|raw_image|image_bytes/.test(routes)) {
    throw new Error("Pantry route source suggests raw receipt media persistence");
}
